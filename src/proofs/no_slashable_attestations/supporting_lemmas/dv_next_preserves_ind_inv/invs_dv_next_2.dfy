...
module Invs_Att_DV_Next_2
{
    ...
    predicate pred_latest_attestation_duty_was_sent_from_dv(
        s': AttDVState,
    ...
    )
    {
        ...
        && var an := s'.sequence_of_attestation_duties_to_be_served[index_next_attestation_duty_to_be_served - 1];
    ...
    requires next_honest_node_after_adding_block_to_bn.requires(s, node, nodeEvent, nodeOutputs, s' );
    requires next_honest_node_after_adding_block_to_bn(s, node, nodeEvent, nodeOutputs, s' );  
    requires nodeEvent.ServeAttestationDuty?
    ensures s'.index_next_attestation_duty_to_be_served > 0
    ensures && pred_latest_attestation_duty_was_sent_from_dv(s', s'.index_next_attestation_duty_to_be_served, nodeEvent.attestation_duty, node )
            && s.index_next_attestation_duty_to_be_served == s'.index_next_attestation_duty_to_be_served - 1;
    {
        ...
        match nodeEvent
        {
            case ServeAttestationDuty(attestation_duty) => 
                assert s.sequence_of_attestation_duties_to_be_served == s'.sequence_of_attestation_duties_to_be_served;
                assert s.index_next_attestation_duty_to_be_served == s'.index_next_attestation_duty_to_be_served - 1;

                var an := s'.sequence_of_attestation_duties_to_be_served[s'.index_next_attestation_duty_to_be_served - 1];
    ...
    ensures s'.index_next_attestation_duty_to_be_served > 0
    ensures && pred_latest_attestation_duty_was_sent_from_dv(s', s'.index_next_attestation_duty_to_be_served, event.event.attestation_duty, event.node )
            && s.index_next_attestation_duty_to_be_served == s'.index_next_attestation_duty_to_be_served - 1;
    {
    ...
    ensures f_serve_attestation_duty(s, attestation_duty).outputs == f_get_empty_AttestationOuputs()
    { }
    ...
    ensures s'.bn.submitted_data == s.bn.submitted_data
    ensures s'.rcvd_attestation_shares == s.rcvd_attestation_shares
    ensures f_check_for_next_duty(s, attestation_duty).outputs == f_get_empty_AttestationOuputs()
    { }
    ...
    ensures f_listen_for_new_imported_blocks(s, block).outputs == f_get_empty_AttestationOuputs()
    { }  
    ...
    lemma lem_f_resend_attestation_shares_unchanged_dvc_vars(
        s: AttDVCState,
        s': AttDVCState
    )
    ...
    ensures forall r, s1, s2 |
            ...
            ::
                s1 == s2
    {
        alem_verify_bls_signature();
    }

    lemma lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_ex_f_listen_for_attestation_shares(
        process: AttDVCState,
        attestation_share: AttestationShare,
        s': AttDVCState,
        dv: AttDVState
                        ...
                        exists hn', att_share: AttestationShare :: inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(dv, hn', att_share, a);
    {
                ...
                attestation_share) 
        {
            ...
            if process.construct_signed_attestation_signature(process.rcvd_attestation_shares[attestation_share.data.slot][k]).isPresent()
            {
                ...
                assert inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(
                                dv,
                                h_s,
                                h_s_att,
    ...
    lemma lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_ex_helper(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )
    requires next_event_preconditions(s, event)
    requires next_event(s, event, s')  
    requires inv_exists_honest_node_that_sent_att_share_for_every_submitted_att(s)
    requires s'.all_attestations_created == s.all_attestations_created
    ensures inv_exists_honest_node_that_sent_att_share_for_every_submitted_att(s');
    {
        forall a |
            && a in s'.all_attestations_created
            && att_signature_is_signed_with_pubkey(a, s'.dv_pubkey)
        ensures exists hn', att_share: AttestationShare :: inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(s', hn', att_share, a);
        {
            var hn', att_share: AttestationShare :|
                inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(s, hn', att_share, a);
            
            assert inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(s', hn', att_share, a);
            assert exists hn', att_share: AttestationShare :: inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(s', hn', att_share, a);
        }
    }    

    lemma lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_dv_next_ReceivedAttestationShare(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )
            ...
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                match nodeEvent
                {
                    case ReceivedAttestationShare(attestation_share) => 
                        assert multiset(f_get_quorum_card<AttestationShare>({attestation_share}, node)) <= s.att_network.messagesInTransit;
                        ...
                        lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_ex_f_listen_for_attestation_shares(
                            s_node,
                            attestation_share,
                            s'_node,
                            s
                        ...
                        ensures exists hn', att_share: AttestationShare :: inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(s', hn', att_share, a);            
                        {
                            ...
                            {
                                var hn', att_share: AttestationShare :| inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(s, hn', att_share, a);
                                assert inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(s', hn', att_share, a);
                            }
                            else 
                            {
                        ...
                        assert inv_exists_honest_node_that_sent_att_share_for_every_submitted_att(s');  
                }
        }
    }

    lemma lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_dv_next_AdversaryTakingStep(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )
            ...
            case AdversaryTakingStep(node, new_attestation_shares_sent, messagesReceivedByTheNode) =>    
                ...
                ensures exists hn', att_share: AttestationShare :: inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(s', hn', att_share, a);      
                {
                    ...
                    else 
                    {
                        ...
                        alem_compute_signing_root_properties<AttestationData>();
                        alem_verify_bls_signature();

                        var a_fork_version := af_bn_get_fork_version(compute_start_slot_at_epoch(a.data.target.epoch));
    ...
    lemma lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_dv_next(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )
    ...
    ensures inv_exists_honest_node_that_sent_att_share_for_every_submitted_att(s')
    {
        ...
        match event 
        {
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                {
                    case ServeAttestationDuty(attestation_duty) => 
                        lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_ex_helper(s, event, s');
                        assert inv_exists_honest_node_that_sent_att_share_for_every_submitted_att(s');  
                    ...
                    case ReceivedAttestationShare(attestation_share) => 
                        lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_dv_next_ReceivedAttestationShare(
                            s,
                            event,
                            s'
                        );
            ...
            case AdversaryTakingStep(node, new_attestation_shares_sent, messagesReceivedByTheNode) =>
                lem_inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_dv_next_AdversaryTakingStep(
                    s,
                    event,
                    s'
                );
    ...
    requires mswr == {}
    ensures f_get_messages_from_messages_with_recipients(mswr) == {}
    {
    }
    ...
    requires s'.att_network.allMessagesSent == 
                    s.att_network.allMessagesSent + f_get_messages_from_messages_with_recipients({});
    ensures  inv_data_of_att_shares_are_decided_values(s');
    {
        ...
        calc {
            ...
            == 
            s.att_network.allMessagesSent + f_get_messages_from_messages_with_recipients(emptySet);
            ==
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_get_s_w_honest_node_states_updated(
        s: AttDVState,
        node: BLSPubkey,
        nodeEvent: AttestationEvent
    ...
    lemma lem_inv_decided_values_of_consensus_instances_are_decided_by_quorum_helper(
        s: AttDVState,
        event: DVAttestationEvent,
        cid: Slot,
        s': AttDVState,
    ...
    ensures decided_value_is_valid(s'.consensus_on_attestation_data[cid]); 
    {
        ...
        var h_nodes :| decided_value_is_based_on_validity_predicates_from_quorum(s_consensus, h_nodes); 
        ...
        {
            assert decided_value_is_valid(s_consensus); 
        ...
        assert decided_value_is_based_on_validity_predicates_from_quorum(s'_consensus, h_nodes); 
        assert decided_value_is_valid(s'_consensus); 
        
    }   

    lemma lem_inv_decided_values_of_consensus_instances_are_decided_by_quorum(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )
    ...
    requires inv_only_dv_construct_signed_attestation_signature(s)
    requires inv_decided_values_of_consensus_instances_are_decided_by_quorum(s)    
    ensures inv_decided_values_of_consensus_instances_are_decided_by_quorum(s')   
    {
        assert s.att_network.allMessagesSent <= s'.att_network.allMessagesSent;
        match event 
        {
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                ensures decided_value_is_valid(s'.consensus_on_attestation_data[cid]);
                {
                    ...
                    {
                        lem_inv_decided_values_of_consensus_instances_are_decided_by_quorum_helper(s, event, cid, s', node, nodeEvent, nodeOutputs);
                    }
                    else
                    {
                        assert decided_value_is_valid(s'.consensus_on_attestation_data[cid]);
                    }
                }
                assert inv_decided_values_of_consensus_instances_are_decided_by_quorum(s');
    ...
    lemma lem_inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k_helper(
        s: AttDVState,
        cid: Slot
    )
    requires inv_decided_values_of_consensus_instances_are_decided_by_quorum(s)    
    requires inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db(s)
    requires inv_all_honest_nodes_is_quorum(s)
    requires inv_unchanged_paras_of_consensus_instances(s)
    requires inv_only_dv_construct_signed_attestation_signature(s)
    requires cid in s.consensus_on_attestation_data.Keys
    ...
    ensures s.consensus_on_attestation_data[cid].decided_value.safe_get().slot == cid
    {
        ...
        var h_nodes_a :| decided_value_is_based_on_validity_predicates_from_quorum(s_consensus, h_nodes_a);
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_get_s_w_honest_node_states_updated_2(
        s: AttDVState,
        node: BLSPubkey,
        nodeEvent: AttestationEvent,
        node': BLSPubkey,
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_helper(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState,
        cid: Slot,
    ...
    requires event.HonestNodeTakingStep?
    requires inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db(s)
    requires inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(s)
    ensures inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_body(cid, attestation_duty, attestation_slashing_db, vp); 
    {
        assert s.att_network.allMessagesSent <= s'.att_network.allMessagesSent;
        match event 
        {
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                var s_w_honest_node_states_updated :=
                    lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_get_s_w_honest_node_states_updated(s, node, nodeEvent);         
            ...
            if hn in s_consensus.honest_nodes_validity_functions.Keys  && vp in s_consensus.honest_nodes_validity_functions[hn]
            {
                ...
                attestation_duty, attestation_slashing_db :| inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_body(cid, attestation_duty, attestation_slashing_db, vp);
            ...
            else 
            {
                ...
                lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_get_s_w_honest_node_states_updated_2(s, node, nodeEvent, vpn, s_w_honest_node_states_updated);
                ...
                assert inv_every_active_consensus_instance_has_corresponding_att_duty_and_validity_predicate(s, vpn, cid);

                attestation_duty, attestation_slashing_db :| inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_body(cid, attestation_duty, attestation_slashing_db, vp);
            }

            assert inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_body(cid, attestation_duty, attestation_slashing_db, vp);
        }

    }  

    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )   
    ...
    requires inv_only_dv_construct_signed_attestation_signature(s)      
    requires inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(s)  
    ensures inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db(s')   
    {
        match event 
        {
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                forall hn, cid: Slot, vp |
                    && hn in s'.consensus_on_attestation_data[cid].honest_nodes_validity_functions.Keys
                    && vp in s'.consensus_on_attestation_data[cid].honest_nodes_validity_functions[hn]
                ensures exists attestation_duty, attestation_slashing_db :: inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_body(cid, attestation_duty, attestation_slashing_db, vp)
                {
                    var attestation_duty: AttestationDuty, attestation_slashing_db := lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_helper(s, event, s', cid, hn, vp);
                }
                assert inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db(s');
    ...
    lemma lem_inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )
    ...
    ensures inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k(s') 
    {
        ...
        lem_inv_decided_values_of_consensus_instances_are_decided_by_quorum(s, event, s');
        lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db(s, event, s');
        lem_inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k2(s');   
    }     

    lemma lem_inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k2(
        s: AttDVState
    )
    requires inv_decided_values_of_consensus_instances_are_decided_by_quorum(s)    
    requires inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db(s)
    ...
    ensures inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k(s) 
    {
        forall cid |
            ...
            && s.consensus_on_attestation_data[cid].decided_value.isPresent()
        {
           lem_inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k_helper(s, cid);
        }        
    }       
    ...
    requires inv_data_of_att_shares_are_decided_values(s)
    requires inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k(s)
    requires inv_attestation_shares_to_broadcast_is_subset_of_all_messages_sent(s)  
    requires inv_all_honest_nodes_is_quorum(s)
    requires inv_unchanged_paras_of_consensus_instances(s)
    requires same_honest_nodes_in_dv_and_ci(s)
    requires inv_only_dv_construct_signed_attestation_signature(s)
    ...
    ensures inv_data_of_att_shares_are_decided_values(s')   
    {
        ...
        assert messagesToBeSent ==  f_multicast(attestation_with_signature_share, process.peers);  
        assert s'.att_network.allMessagesSent == 
                    s.att_network.allMessagesSent + f_get_messages_from_messages_with_recipients(messagesToBeSent); 
        ...
        ensures s'.consensus_on_attestation_data[att_share.data.slot].decided_value.safe_get() == att_share.data;     
        {
            ...
            else
            {
                ...
                lem_inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k(s, event, s');   
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dvc_f_update_att_consensus_instance_validity_check_states(
    ...
    requires forall k | k in m.Keys :: inv_exists_slashing_db_for_given_att_duty_and_vp(k, m[k].attestation_duty, m[k].validityPredicate)
    ensures forall k | k in m'.Keys :: inv_exists_slashing_db_for_given_att_duty_and_vp(k, m'[k].attestation_duty, m'[k].validityPredicate)    
    {
        ...
        forall k | k in m'.Keys
        ensures inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_body(k, m'[k].attestation_duty, new_attestation_slashing_db, m'[k].validityPredicate); 
        {
            assert m'[k] == m[k].(
                    validityPredicate := (ad: AttestationData) => ci_decision_att_signature_is_signed_with_pubkey_data(new_attestation_slashing_db, ad, m[k].attestation_duty)
            );
            assert  inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_body(k, m'[k].attestation_duty, new_attestation_slashing_db, m'[k].validityPredicate);        
        }
    }

    
    lemma lem_inv_db_of_validity_predicate_contains_all_previous_decided_values_f_check_for_next_duty_f_update_att_consensus_engine_state(
    ...
    {
        lem_f_update_att_consensus_instance_validity_check_states(s.active_consensus_instances, new_attestation_slashing_db, s'.active_consensus_instances);
    ...
    lemma lem_inv_db_of_validity_predicate_contains_all_previous_decided_values_f_check_for_next_duty_f_update_att_consensus_engine_state2(
        s: ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
        new_attestation_slashing_db: set<SlashingDBAttestation>,
        s': ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
        slot: Slot,
    ...
    // lemma lem_inv_db_of_validity_predicate_contains_all_previous_decided_values_f_check_for_next_duty_f_update_att_consensus_engine_state3(
    //     s: ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
    //     new_attestation_slashing_db: set<SlashingDBAttestation>,
    //     s': ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
    //     slot: Slot,
    ...
    lemma lem_inv_db_of_validity_predicate_contains_all_previous_decided_values_f_check_for_next_duty_f_update_att_consensus_engine_state4(
        s: ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
        new_attestation_slashing_db: set<SlashingDBAttestation>,
        s': ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
        slot: Slot,
    ...
    lemma lem_inv_db_of_validity_predicate_contains_all_previous_decided_values_f_check_for_next_duty_f_update_att_consensus_engine_state5(
        s: ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
        new_attestation_slashing_db: set<SlashingDBAttestation>,
        s': ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
        slot: Slot    
    ...
    lemma lem_inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate_f_terminate_current_attestation_duty(
        process: AttDVCState,
        process': AttDVCState
    )
    ...
    requires inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(process)
    ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(process')
    { }       

    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dvc_f_start_next_duty(
        process: AttDVCState,
        attestation_duty: AttestationDuty,
        process': AttDVCState
    )
    ...
    ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(process')
    {
        ...
        ensures inv_exist_att_duty_and_validity_predicate_for_active_consensus_instance_at_slot_cid(new_process, cid) 
        { 
            if cid in process.attestation_consensus_engine_state.active_consensus_instances.Keys
            {
                assert inv_exist_att_duty_and_validity_predicate_for_active_consensus_instance_at_slot_cid(new_process, cid);
            }
            else
            {
                ...
                var acvc := AttestationConsensusValidityCheckState(
                    ...
                    validityPredicate := (ad: AttestationData) => ci_decision_att_signature_is_signed_with_pubkey_data(process.attestation_slashing_db, ad, attestation_duty)
                );

                assert acvc == process'.attestation_consensus_engine_state.active_consensus_instances[cid];
                assert inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_body(cid, attestation_duty, process.attestation_slashing_db, acvc.validityPredicate);    
                assert inv_exists_slashing_db_for_given_att_duty_and_vp(cid, attestation_duty, acvc.validityPredicate);
                assert inv_exist_att_duty_and_validity_predicate_for_active_consensus_instance_at_slot_cid(new_process, cid);
            }
        }
    }
    
    lemma lem_inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate_f_check_for_next_duty(
        process: AttDVCState,
        attestation_duty: AttestationDuty,
        process': AttDVCState
    )
    ...
    ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(process'); 
    {
        ...
        {                
            var new_process := f_new_process_after_f_update_att_consensus_engine_state(process, attestation_duty);

            lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dvc_f_update_att_consensus_instance_validity_check_states(
        ...
        {
            lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dvc_f_start_next_duty(
                process,
                attestation_duty,
                process'
            );                
    ...
    lemma lem_inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate_f_serve_attestation_duty(
        process: AttDVCState,
        attestation_duty: AttestationDuty,
        process': AttDVCState
    )
    ...
    ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(process'); 
    {
        ...
        lem_inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate_f_terminate_current_attestation_duty(
            process_rcvd_duty,
            process_after_stopping_active_consensus_instance
        );
        lem_inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate_f_check_for_next_duty(
            process_after_stopping_active_consensus_instance,
            attestation_duty,
            process'
        );        
    ...
    lemma lem_inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate_f_att_consensus_decided(
        process: AttDVCState,
        id: Slot,
        decided_attestation_data: AttestationData,        
        s': AttDVCState
    ...
    requires inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(process) 
    ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(s'); 
    {
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_f_listen_for_new_imported_blocks(
        process: AttDVCState,
        block: BeaconBlock,
        s': AttDVCState
    )
    ...
    ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(s');     
    {
        ...
        var process := f_f_stop_att_consensus_instances_after_receiving_new_imported_blocks(
                                process,
                                block
                            );
        ...
        {
            var s_mod := f_f_update_att_consensus_engine_state_in_listen_for_new_imported_blocks(
                                    process,
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next_ServeAttestationDuty(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )   
    ...
    requires inv_only_dv_construct_signed_attestation_signature(s)      
    requires inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(s)  
    ensures inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(s')    
    {
        match event 
        {
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                match nodeEvent
                {
                    case ServeAttestationDuty(attestation_duty) => 
                        ...
                        ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(s'.honest_nodes_states[n]); 
                        {
                            if n == node
                            {
                                lem_inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate_f_serve_attestation_duty(s_node, attestation_duty, s'_node);
                                assert inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(s'_node); 
                            }
                        }
                        assert inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(s');                        
                }
        }          
    }

    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next_AttConsensusDecided(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )   
                ...
                match nodeEvent
                {
                    case AttConsensusDecided(id, decided_attestation_data) => 
                        ...
                        ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(s'.honest_nodes_states[n]); 
                        {
                            if n == node
                            {
                                lem_inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate_f_att_consensus_decided(s_node, id, decided_attestation_data, s'_node);
                                assert inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(s'_node); 
                            }
                        }
                        assert inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(s');    
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next_ReceivedAttestationShare(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )   
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next_ImportedNewBlock(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )   
                ...
                match nodeEvent
                {
                    case ImportedNewBlock(block) => 
                        ...
                        ensures inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(s'.honest_nodes_states[n]); 
                        {
                            ...
                            {
                                var s_node := f_add_block_to_bn(s_node, nodeEvent.block);
                                lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_f_listen_for_new_imported_blocks(s_node, block, s'_node);
                                assert inv_every_active_consensus_instance_of_dvc_has_corresponding_att_duty_and_validity_predicate(s'_node); 
                            }
                        }
                        assert inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(s');          
    ...
    lemma lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next(
        s: AttDVState,
        event: DVAttestationEvent,
        s': AttDVState
    )   
    ...
    ensures inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(s')    
    {
        match event 
        {
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                {
                    case ServeAttestationDuty(attestation_duty) => 
                        lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next_ServeAttestationDuty(
                            s,
                            event,
                            s'
                        );  
                    ...
                    case AttConsensusDecided(id, decided_attestation_data) => 
                        lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next_AttConsensusDecided(
                            s,
                            event,
                            s'
                        );  
                    ...
                    case ReceivedAttestationShare(attestation_share) => 
                        lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next_ReceivedAttestationShare(
                            s,
                            event,
                            s'
                        );    
                    ...
                    case ImportedNewBlock(block) => 
                        lem_inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv_next_ImportedNewBlock(
                            s,
                            event,
                            s'
                        );                        
                ...
                ;  
    {
            ...
            ;         
        {
            ...
            var hn', att_share: AttestationShare :| inv_exists_honest_node_that_sent_att_share_for_every_submitted_att_body(dv, hn', att_share, a');
    ...
    ensures inv_data_of_att_shares_are_decided_values(s')   
    {
        match event 
        {
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                {
                    case ResendAttestationShares => 
                        var messagesToBeSent := f_resend_attestation_shares(s_node).outputs.att_shares_sent;     
