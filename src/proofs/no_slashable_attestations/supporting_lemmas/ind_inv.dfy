...
module Att_Ind_Inv_With_Empty_Init_Att_Slashing_DB
{
    ...
    predicate ind_inv(dv: AttDVState)       
    {
        && invs_group_1(dv)        
        && invs_group_2(dv)
        && invs_group_3(dv)
    ...
    predicate invs_group_1(dv: AttDVState)       
    {
    ...
    predicate invs_group_2(dv: AttDVState)       
    {        
        ...
        &&  assump_seq_of_att_duties_is_in_order_of_slots(dv.sequence_of_attestation_duties_to_be_served)              
    }

    predicate invs_group_3(dv: AttDVState)       
    {        
        &&  inv_no_duplicated_att_duties(dv)           
        &&  inv_every_att_duty_before_dv_att_index_was_delivered(dv) 
        &&  inv_no_active_consensus_instances_before_first_att_duty_was_received(dv)              
    }

    predicate invs_group_4(dv: AttDVState)       
    {        
        &&  inv_slots_of_active_consensus_instances_are_not_higher_than_slot_of_latest_att_duty(dv)  
        &&  inv_dvc_has_corresponding_att_duty_for_every_active_attestation_consensus_instance(dv)          
        &&  inv_consensus_instance_indexed_k_is_for_rcvd_duty_at_slot_k(dv)  
        &&  inv_rcvd_att_duties_are_from_dv_seq_of_att_duties(dv)
    }

    predicate invs_group_5(dv: AttDVState)       
    {                
        &&  inv_slashing_db_hist_keeps_track_only_of_rcvd_att_duties(dv)  
        &&  inv_exist_db_in_slashing_db_hist_and_att_duty_for_every_validity_predicate(dv)  
        &&  inv_current_validity_predicate_for_slot_k_is_stored_in_slashing_db_hist_k(dv)          
    }

    predicate invs_group_6(dv: AttDVState)       
    {                
        &&  inv_every_db_in_slashing_db_hist_is_subset_of_att_slashing_db(dv)  
        &&  inv_active_att_consensus_instances_are_tracked_in_slashing_db_hist(dv)
        &&  inv_only_dv_construct_signed_attestation_signatures(dv)        
    }

    predicate invs_group_7(dv: AttDVState)       
    {                
    ...
    predicate invs_group_8(dv: AttDVState)       
    {                
        ...
        && inv_decided_values_of_consensus_instances_are_decided_by_quorum(dv)    
        && inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db_for_dv(dv)
        && inv_every_sent_validity_predicate_is_based_on_rcvd_att_duty_and_slashing_db(dv)
        
    }

    predicate invs_group_9(dv: AttDVState)       
    {                    
        && inv_decided_value_of_consensus_instance_for_slot_k_is_for_slot_k(dv) 
        && inv_in_transit_messages_are_in_allMessagesSent(dv)
        && inv_attestation_shares_to_broadcast_are_sent_messages(dv)
        && inv_attestation_shares_to_broadcast_is_subset_of_all_messages_sent(dv)
    }

    predicate invs_group_10(dv: AttDVState)       
    {                
        && inv_exists_att_duty_in_dv_seq_of_att_duties_for_every_slot_in_slashing_db_hist(dv)        
        && inv_latest_att_duty_is_from_dv_seq_of_att_duties(dv)
        && inv_slots_of_consensus_instances_are_up_to_slot_of_latest_att_duty(dv)        
        && inv_data_of_att_shares_are_decided_values(dv)                     
    }
    
    predicate invs_group_11(dv: AttDVState)       
    {                
        && inv_future_decisions_known_by_dvc_are_decisions_of_quorums(dv)
        && inv_slots_in_future_decided_data_are_correct(dv)  
    }
    
    predicate invs_group_12(dv: AttDVState)       
    {                
        && inv_sent_validity_predicates_are_only_for_slots_stored_in_slashing_db_hist(dv)
        && inv_all_validity_predicates_are_stored_in_slashing_db_hist(dv)
        && inv_every_consensus_instance_condition_for_safety_is_true(dv)
    ...
    predicate invs_group_13(dv: AttDVState)       
    {   
        && inv_every_decided_data_has_honest_witness(dv)
        && inv_all_created_attestations_are_valid(dv)
        && inv_attestation_is_created_based_on_shares_from_quorum_of_dvc_signer_pubkeys(dv)
        && inv_unchanged_dvc_rs_pubkey(dv)
    }

    predicate invs_group_14(dv: AttDVState)       
    {   
        && assump_seq_of_att_duties_is_in_order_of_slots(dv.sequence_of_attestation_duties_to_be_served)
