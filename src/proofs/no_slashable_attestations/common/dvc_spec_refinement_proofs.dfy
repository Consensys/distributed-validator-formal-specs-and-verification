...
module Spec_Spec_NonInstr_Refinement
{
    ...
    predicate consensusEngineStateRel(
        cei: Consensus_Engine_Instr.ConsensusEngineState<AttestationConsensusValidityCheckState, AttestationData, SlashingDBAttestation>,
        ceni: Consensus_Engine_NonInstr.ConsensusEngineState<AttestationConsensusValidityCheckState>
    )
    {
        ...
        if dvci.current_attestation_duty.isPresent()
        {
            var dvci_new :=
                    dvci.(
                        ...
                        attestation_consensus_engine_state := Consensus_Engine_Instr.stopAttConsensusInstances(
            ...
            var dvcni_new :=
                    dvcni.(
                        current_attestation_duty := None,
                        attestation_consensus_engine_state := Consensus_Engine_NonInstr.stopAttConsensusInstances(
        ...
        if attestation_duty.slot in dvci.future_att_consensus_instances_already_decided.Keys
        {
            ...
            var dvci_new := 
                    dvci.(
                        ...
                        attestation_consensus_engine_state := Consensus_Engine_Instr.updateAttConsensusInstanceValidityCheck(
                            dvci.attestation_consensus_engine_state,
            ...
            var dvcni_new := dvcni.(
                ...
                attestation_slashing_db := new_attestation_slashing_db,
                attestation_consensus_engine_state := Consensus_Engine_NonInstr.updateAttConsensusInstanceValidityCheck(
                    dvcni.attestation_consensus_engine_state,
                    new_attestation_slashing_db
                )
            ); 
    ...
    );     
    {
        ...
        var dvci := f_stopAttConsensusInstances_after_receiving_new_imported_blocks(
                                dvci,
                                block
                            );
                    ...
                    )
        {
            ...
            var dvci := f_updateAttConsensusInstanceValidityCheck_in_listen_for_new_imported_blocks(
                                    dvci,
                                    att_consensus_instances_already_decided
                                );
