...
module Block_Ind_Inv_With_Empty_Initial_Block_Slashing_DB
{
    ...
    predicate ind_inv(dv: BlockDVState)       
    {
        && invs_group_1(dv)        
        && invs_group_2(dv)
        && invs_group_3(dv)
    ...
    predicate invs_group_1(dv: BlockDVState)       
    {
    ...
    predicate invs_group_2(dv: BlockDVState)       
    {        
    ...
    predicate invs_group_3(dv: BlockDVState)       
    {        
        &&  inv_no_duplicated_proposer_duties_in_dv_seq(dv)           
    ...
    predicate invs_group_4(dv: BlockDVState)       
    {        
        &&  inv_slots_of_active_consensus_instances_are_not_higher_than_slot_of_latest_proposer_duty(dv)  
        &&  inv_dvc_only_joins_consensus_instances_for_which_it_has_received_corresponding_proposer_duties(dv)          
        &&  inv_consensus_instance_indexed_k_is_for_rcvd_duty_at_slot_k(dv)  
        &&  inv_rcvd_proposer_duty_is_from_dv_seq_for_proposer_duties(dv)
    }

    predicate invs_group_5(dv: BlockDVState)       
    {                
        &&  inv_slashing_db_hist_keeps_track_of_all_rcvd_proposer_duties(dv)  
    ...
    predicate invs_group_6(dv: BlockDVState)       
    {                
        &&  inv_every_db_in_slashing_db_hist_is_subset_of_block_slashing_db(dv)  
        &&  inv_active_consensus_instances_are_tracked_in_slashing_db_hist(dv)
    }

    predicate invs_group_7(dv: BlockDVState)       
    {                
        // &&  inv_same_node_status_between_dv_and_ci(dv)
    ...
    predicate invs_group_8(dv: BlockDVState)       
    {   
    ...
    predicate invs_group_9(dv: BlockDVState)       
    {                    
        ...
        && inv_block_shares_to_broadcast_is_subset_of_all_sent_messages(dv)

        
    }

    predicate invs_group_10(dv: BlockDVState)       
    {   
        && inv_available_latest_proposer_duty_is_from_dv_seq_of_proposer_duties(dv)        
        && inv_exists_proposer_duty_in_dv_seq_of_proposer_duties_for_every_slot_in_slashing_db_hist(dv) 
    ...
    predicate invs_group_11(dv: BlockDVState)       
    {             
    ...
    predicate invs_group_12(dv: BlockDVState)       
    {   
        && inv_consensus_instance_condition_for_safety_is_true(dv)
    ...
    predicate invs_group_13(dv: BlockDVState)       
    {   
        && inv_unchanged_dvc_rs_pubkey(dv)
        && inv_all_created_signed_beacon_blocks_are_valid(dv)
        && inv_complete_signed_block_is_created_based_on_shares_from_quorum(dv)
        && inv_exists_honest_dvc_as_witness_for_every_decided_beacon_block(dv)

        // Move to intermediate steps
        // && inv_proposer_duty_in_next_delivery_is_higher_than_latest_served_proposer_duty(dv)
    }

    predicate invs_group_14(dv: BlockDVState)       
    {   
        ...
        && inv_slots_of_consensus_instances_are_up_to_slot_of_latest_proposer_duty(dv)
        && inv_db_of_vp_contains_all_beacon_block_of_sent_block_shares_with_lower_slots(dv)
    }

    predicate invs_group_15(dv: BlockDVState)       
    {   
        ...
        && inv_slots_in_slashing_db_are_not_higher_than_slot_of_latest_slashing_db_block(dv)
