...
module RS_Axioms
{
    ...
    ensures forall ad1, fv1, sr1, pk1, ad2, fv2, sr2, pk2 ::
            ...
            && af_rs_sign_attestation(ad1, fv1, sr1, pk1) == af_rs_sign_attestation(ad2, fv2, sr2, pk2) 
            ==>
            && sr1 == sr2 
            && pk1 == pk2         


    function {:axiom} af_rs_sign_block(        
        block: BeaconBlock,
        fork_version: Version,
        signing_root: Root,
        rs: RSState
    ...
    lemma {:axiom} alem_rs_block_sign_and_verification_propeties()
    ensures forall beacon_block, fork_version, signing_root, rs |
                    af_rs_sign_block.requires(beacon_block, fork_version, signing_root, rs) ::
                    verify_bls_signature(
                        signing_root,
                        af_rs_sign_block(beacon_block, fork_version, signing_root, rs),
                        rs.pubkey
                    )
    ensures forall signing_root, signature, pubkey ::
        verify_bls_signature(signing_root, signature, pubkey) <==>
            ...
            var rs := RSState(pubkey);
            && af_rs_sign_block.requires(beacon_block, fork_version, signing_root, rs)
            && signature == af_rs_sign_block(beacon_block, fork_version, signing_root, rs)

    ensures forall ad1, fv1, sr1, pk1, ad2, fv2, sr2, pk2 ::
            && af_rs_sign_block.requires(ad1, fv1, sr1, pk1)
            && af_rs_sign_block.requires(ad2, fv2, sr2, pk2)
            && af_rs_sign_block(ad1, fv1, sr1, pk1) == af_rs_sign_block(ad2, fv2, sr2, pk2) 
            ==>
            && sr1 == sr2 
            && pk1 == pk2  

    // Don't need to use fork_version
    function {:axiom} af_rs_sign_randao_reveal(        
        epoch: Epoch,
        fork_version: Version,
        signing_root: Root,
        rs: RSState
    ...
    lemma {:axiom} alem_compute_signing_root_properties<T>()
    ...
    lemma {:axiom} alem_verify_bls_signature()
    ensures 
