...
module Invs_DV_Next_1
{
    ...
    lemma lem_inv_queued_att_duty_is_dvn_seq_of_att_duty_dv_next_helper(
        dv: DVState,
        event: DV.Event,
        dv': DVState
    )
    ...
    lemma lem_inv_queued_att_duty_is_dvn_seq_of_att_duty_dv_next(
        dv: DVState,
        event: DV.Event,
        dv': DVState
    )
            ...
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                match nodeEvent
                {
                    ...
                    case ReceivedAttestationShare(attestation_share) =>                          
                        ...
                        lem_inv_queued_att_duty_is_dvn_seq_of_att_duty_dv_next_helper(dv, event, dv');                   
    ...
    lemma lem_inv_queued_att_duty_is_rcvd_duty_dv_next(
        dv: DVState,
        event: DV.Event,
        dv': DVState
    )    
            ...
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                match nodeEvent
                {
                    case ServeAttstationDuty(attestation_duty) =>     
                        lem_inv_queued_att_duty_is_rcvd_duty_f_serve_attestation_duty(dvc, attestation_duty, dvc');
                        
                    case AttConsensusDecided(id, decided_attestation_data) => 
                        lem_inv_queued_att_duty_is_rcvd_duty_f_att_consensus_decided(dvc, id, decided_attestation_data, dvc');                        
                        
                    case ReceivedAttestationShare(attestation_share) =>                         
                        lem_inv_queued_att_duty_is_rcvd_duty_f_listen_for_attestation_shares(dvc, attestation_share, dvc');                        
   
                    case ImportedNewBlock(block) => 
                        var dvc := add_block_to_bn(dvc, nodeEvent.block);
                        lem_inv_queued_att_duty_is_rcvd_duty_f_listen_for_new_imported_blocks(dvc, block, dvc');                        
    ...
    lemma lem_inv_not_none_current_att_duty_is_latest_served_att_duty_dv_next(
        dv: DVState,
        event: DV.Event,
        dv': DVState
    )    
    ...
    {        
        match event 
        {
            case HonestNodeTakingStep(node, nodeEvent, nodeOutputs) =>
                ...
                {
                    case ServeAttstationDuty(attestation_duty) =>                             
                        lem_inv_not_none_current_att_duty_is_latest_served_att_duty_f_serve_attestation_duty(dvc, attestation_duty, dvc');
                        
                    case AttConsensusDecided(id, decided_attestation_data) => 
                        lem_inv_not_none_current_att_duty_is_latest_served_att_duty_f_att_consensus_decided(dvc, id, decided_attestation_data, dvc');                                                
                        
                    case ReceivedAttestationShare(attestation_share) =>                         
                        lem_inv_not_none_current_att_duty_is_latest_served_att_duty_f_listen_for_attestation_shares(dvc, attestation_share, dvc');                        
   
                    case ImportedNewBlock(block) => 
                        var dvc_mod := add_block_to_bn(dvc, nodeEvent.block);
                        lem_inv_not_none_current_att_duty_is_latest_served_att_duty_add_block_to_bn(dvc, nodeEvent.block, dvc_mod);
                        assert inv_not_none_current_att_duty_is_latest_served_att_duty_body(dvc_mod);
                        lem_inv_not_none_current_att_duty_is_latest_served_att_duty_f_listen_for_new_imported_blocks(dvc_mod, block, dvc');                        
                        assert inv_not_none_current_att_duty_is_latest_served_att_duty_body(dvc');
