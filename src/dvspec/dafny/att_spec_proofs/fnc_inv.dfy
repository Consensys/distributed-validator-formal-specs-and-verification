...
/*
    lemma lemma_inv19_f_listen_for_attestation_shares(
        process: DVCNodeState,
        attestation_share: AttestationShare,
        process': DVCNodeState
    )
    ...
    lemma lemma_inv19_f_resend_attestation_share(
        process: DVCNodeState,
        process': DVCNodeState)
    requires f_resend_attestation_share.requires(process)
    requires process' == f_resend_attestation_share(process).state        
    ...
    lemma lemma_inv19_add_block_to_bn(
        s: DVCNodeState,
        block: BeaconBlock,
        s': DVCNodeState 
    )
    requires add_block_to_bn.requires(s, block)
    requires s' == add_block_to_bn(s, block)    
    requires inv19_body(s)
    ensures inv19_body(s')
    { }

    lemma lemma_inv19_f_start_next_duty(process: DVCNodeState, attestation_duty: AttestationDuty, process': DVCNodeState)
    requires f_start_next_duty.requires(process, attestation_duty)
    requires process' == f_start_next_duty(process, attestation_duty).state   
    requires forall queued_duty: AttestationDuty | queued_duty in process.attestation_duties_queue ::
                        attestation_duty.slot <= queued_duty.slot                              
    ...
    lemma lemma_inv19_f_listen_for_new_imported_blocks(
        process: DVCNodeState,
        block: BeaconBlock,
        process': DVCNodeState
    )
    ...
    ensures inv19_body(process')
    {
        ...
        if process.current_attestation_duty.isPresent() && process.current_attestation_duty.safe_get().slot in att_consensus_instances_already_decided
        {
            ...
            lemma_inv19_f_check_for_next_queued_duty(process, process');
        }
        else
        {               
            assert inv19_body(process);
    ...
    lemma lemma_inv19_f_att_consensus_decided(
        process: DVCNodeState,
        id: Slot,
        decided_attestation_data: AttestationData, 
        process': DVCNodeState
    ...
    ensures inv19_body(process')
    {
        ...
        lemma_inv19_f_check_for_next_queued_duty(process, ret_check_for_next_queued_duty.state);
