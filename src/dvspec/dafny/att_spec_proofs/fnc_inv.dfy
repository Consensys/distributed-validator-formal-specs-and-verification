...
module Fnc_Inv
{
    ...
    lemma lemma_inv14_f_start_next_duty(process: DVCNodeState, attestation_duty: AttestationDuty, process': DVCNodeState)
    requires f_start_next_duty.requires(process, attestation_duty)
    requires process' == f_start_next_duty(process, attestation_duty).state        
    requires inv8_body(process) || inv14_body(process)
    ensures inv14_body(process')
    { }  

    lemma lemma_inv14_f_check_for_next_queued_duty(
        process: DVCNodeState,
        process': DVCNodeState
    )
    requires f_check_for_next_queued_duty.requires(process)
    ...
    decreases process.attestation_duties_queue
    {
        ...
        {            
                if process.attestation_duties_queue[0].slot in process.future_att_consensus_instances_already_decided.Keys 
                {
                    ...
                    lemma_inv14_f_check_for_next_queued_duty(process_mod, process');
                }
                else
                { 
                    var process_mod := process.(
                    ...
                    assert inv8_body(process_mod) || inv14_body(process_mod);

                    lemma_inv14_f_start_next_duty(process_mod, process.attestation_duties_queue[0], process');
                }
        }
        else
        { 
    ...
    lemma lemma_inv14_f_serve_attestation_duty(
        process: DVCNodeState,
        attestation_duty: AttestationDuty,
        process': DVCNodeState
    )  
    ...
    lemma lemma_inv14_f_att_consensus_decided(
        process: DVCNodeState,
        id: Slot,
        decided_attestation_data: AttestationData, 
        process': DVCNodeState
    ...
    ensures inv14_body(process')
    {
        ...
        assert inv14_body(process);

        var ret_check_for_next_queued_duty := f_check_for_next_queued_duty(process);
        
        lemma_inv14_f_check_for_next_queued_duty(process, ret_check_for_next_queued_duty.state);

        assert process' == ret_check_for_next_queued_duty.state;
        
    }  

    lemma lemma_inv14_f_listen_for_attestation_shares(
        process: DVCNodeState,
        attestation_share: AttestationShare,
        process': DVCNodeState
    )
    ...
    lemma lemma_inv14_f_listen_for_new_imported_blocks(
        process: DVCNodeState,
        block: BeaconBlock,
        process': DVCNodeState
    )
    ...
    ensures inv14_body(process')
    {
        ...
        if process.current_attestation_duty.isPresent() && process.current_attestation_duty.safe_get().slot in att_consensus_instances_already_decided
        {
            ...
            lemma_inv14_f_check_for_next_queued_duty(process, process');
        }
        else
        {   
            assert inv14_body(process);
    ...
    lemma lemma_inv14_f_resend_attestation_share(
        process: DVCNodeState,
        process': DVCNodeState)
    requires f_resend_attestation_share.requires(process)
    requires process' == f_resend_attestation_share(process).state    
    ...
    lemma lemma_inv14_add_block_to_bn(
        s: DVCNodeState,
        block: BeaconBlock,
        s': DVCNodeState 
    )
    requires add_block_to_bn.requires(s, block)
    requires s' == add_block_to_bn(s, block)
    requires inv14_body(s)
    ensures inv14_body(s')
    { }

    lemma lemma_inv15_f_start_next_duty(process: DVCNodeState, attestation_duty: AttestationDuty, process': DVCNodeState)
    requires f_start_next_duty.requires(process, attestation_duty)
    requires process' == f_start_next_duty(process, attestation_duty).state        
    requires inv15_body(process)
    ensures inv15_body(process')
    { }  

    lemma lemma_inv15_f_check_for_next_queued_duty(
        process: DVCNodeState,
        process': DVCNodeState
    )
    requires f_check_for_next_queued_duty.requires(process)
    ...
    decreases process.attestation_duties_queue
    {
        ...
        {            
                if process.attestation_duties_queue[0].slot in process.future_att_consensus_instances_already_decided.Keys 
                {
                    ...
                    lemma_inv15_f_check_for_next_queued_duty(process_mod, process');
                }
                else
                { 
                    var process_mod := process.(
                    ...
                    assert inv8_body(process_mod) || inv15_body(process_mod);

                    lemma_inv15_f_start_next_duty(process_mod, process.attestation_duties_queue[0], process');
                }
        }
        else
        { 
    ...
    lemma lemma_inv15_f_listen_for_attestation_shares(
        process: DVCNodeState,
        attestation_share: AttestationShare,
        process': DVCNodeState
    )
    ...
    lemma lemma_inv15_f_listen_for_new_imported_blocks(
        process: DVCNodeState,
        block: BeaconBlock,
        process': DVCNodeState
    )
    ...
    ensures inv15_body(process')
    {
        ...
        if process.current_attestation_duty.isPresent() && process.current_attestation_duty.safe_get().slot in att_consensus_instances_already_decided
        {
            ...
            lemma_inv15_f_check_for_next_queued_duty(process, process');
        }
        else
        {   
            assert inv15_body(process);
    ...
    lemma lemma_inv15_f_resend_attestation_share(
        process: DVCNodeState,
        process': DVCNodeState)
    requires f_resend_attestation_share.requires(process)
    requires process' == f_resend_attestation_share(process).state    
    ...
    lemma lemma_inv15_add_block_to_bn(
        s: DVCNodeState,
        block: BeaconBlock,
        s': DVCNodeState 
    )
    requires add_block_to_bn.requires(s, block)
    requires s' == add_block_to_bn(s, block)
    requires inv15_body(s)
    ensures inv15_body(s')
    { }

    lemma lemma_inv15_f_att_consensus_decided(
        process: DVCNodeState,
        id: Slot,
        decided_attestation_data: AttestationData, 
        process': DVCNodeState
    ...
    ensures inv15_body(process')
    {
        ...
        lemma_inv15_f_check_for_next_queued_duty(process, ret_check_for_next_queued_duty.state);

        assert process' == ret_check_for_next_queued_duty.state;
        
    }  

    lemma lemma_inv15_f_serve_attestation_duty(
        process: DVCNodeState,
        attestation_duty: AttestationDuty,
        process': DVCNodeState
    )  
    ...
    requires process' == f_serve_attestation_duty(process, attestation_duty).state
    requires inv5_body(process)  
    requires inv13_body(process, attestation_duty)  
    requires inv15_body(process)
    ensures inv15_body(process')
    {
        var process_mod := process.(
